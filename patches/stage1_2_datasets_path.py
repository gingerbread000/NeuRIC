    ...
    def _regularize_images(self, images: list["ImageInput"], **kwargs) -> dict[str, list["ImageObject"]]:
        ...
        for image in images:
            if isinstance(image, (str, BinaryIO)):
                ...
                if "b2nd_path" in image:
                    image = blosc2.open(image["b2nd_path"])[image['ind']]
    ...
    def _regularize_videos(
        ...
        for video in videos:
            if video.endswith('.npz'):
                video_stream = np.load(video)["arr_0"]
                if len(video_stream.shape) == 3:
                    video_stream = np.array([
                        cv2.cvtColor(frame, cv2.COLOR_GRAY2RGB) for frame in video_stream
                    ])
                elif len(video_stream.shape) == 4:
                    video_stream = np.transpose(video_stream, (1, 2, 3, 0))

                if np.random.random() < 0.3:
                    video_stream = np.flip(video_stream, axis=1)
                if np.random.random() < 0.3:
                    video_stream = np.flip(video_stream, axis=2)

                assert len(video_stream.shape) == 4 and video_stream.shape[-1] == 3
                frames: list[ImageObject] = []
                ...
                fps_per_video.append(len(frames))
            elif video.endswith('.b2nd'):
                # video_stream = iio.imread(video)
                video_stream = blosc2.open(video)
                video_stream = video_stream[:]
                video_stream = (video_stream - video_stream.min()) / (video_stream.max() - video_stream.min() + 1e-8) * 255
                assert len(video_stream.shape) == 3
                frames: list[ImageObject] = []
                for frame_idx, frame in enumerate(video_stream):
                    if frame.shape[0] > 60:
                        if frame_idx % 2 == 0:
                            frames.append(Image.fromarray(frame).convert("RGB").resize((224,224)))
                    else:
                        frames.append(Image.fromarray(frame).convert("RGB").resize((224,224)))
    ...
    def _get_mm_inputs(
        ...
        if len(videos) != 0:
            video_data = self._regularize_videos(
                ...
                image_max_pixels=getattr(processor, "video_max_pixels", 256 * 256),
                image_min_pixels=getattr(processor, "video_min_pixels", 16 * 16),
                video_fps=getattr(processor, "video_fps", 2.0)*4,
                video_maxlen=getattr(processor, "video_maxlen", 128),
            )
